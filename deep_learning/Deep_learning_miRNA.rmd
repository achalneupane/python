...
for fold in range(9, nfolds):
  ...
  with open("tmp/negative_sequences.fold", "w") as oneg, open("tmp/positive_sequences.fold", "w") as opos, open("tmp/test_sequences.fold", "w") as otest: 
    ptrain, ptest = split_fasta(npos, test_pos_ind, "dataset/sequences_virus_miRNAs.fold", otest, opos)
...
for i, imb in enumerate(imbalances):
  ...
  if i == 0:
    legend = "auto"
  sns.lineplot(data=df[df.imb==imb], x="recall", y="precision", hue="model", ax=ax[i], legend=legend)
